...
from cancer_ai.validator.model_db import ModelPersister, ModelInfoTable, Base
...
def model_persister(mock_subtensor, db_session):
    ...
    persister = ModelPersister(mock_subtensor, db_url='sqlite:///:memory:')
...
def model_persister_fixed(fixed_mock_subtensor, db_session):
    ...
    persister = ModelPersister(fixed_mock_subtensor, db_url='sqlite:///:memory:')
    persister.Session = mock.Mock(return_value=db_session)
    return persister
...
def test_add_model(model_persister, mock_chain_miner_model, db_session):
    ...
    session = db_session
    model_record = session.query(ModelInfoTable).first()
...
def test_clean_old_records(model_persister, mock_chain_miner_model, db_session):
    ...
    records = session.query(ModelInfoTable).filter_by(hotkey="mock_hotkey").all()
